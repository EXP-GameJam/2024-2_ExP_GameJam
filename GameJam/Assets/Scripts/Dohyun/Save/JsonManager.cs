...
public class JsonManager : ManagerSingle<JsonManager>, IInit // Json데이터를 읽고 쓰는 Manager이다
{
    ...
    public void Save() // 데이터 저장! SaveData형식으로 Resources/Data 에 저장된다. 형식은 SaveData를 따라감
    {
        SaveJson(SaveData);
    }

    public void Load() // 데이터를 불러온다! 한번만 해도 된다. 어처피 Init할때 = 최초 1회 JsonManager가 실행될때 불러온다.
    {
        LoadJsonData<SaveData>("SaveData", out SaveData);
    }
